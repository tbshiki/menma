...
Markdown を書いてブラウザで発表できる、軽量な Web スライドシステムを作る。要件・基本設計・実装計画は `markdown-web-slides-project-spec.md` が正典。実装方針で迷ったら、まずこの仕様書を読む。

想定スタックは Vite + TypeScript + markdown-it + CSS + Playwright。MVP では React / Vue / Svelte などの UI フレームワークを使わない。依存を増やす変更は、仕様書の方針に照らして理由を示してから行う。

### 独立実装の原則（最重要）

このプロジェクトは、既存のスライドツールの「別実装」であって複製ではない。仕様書 2 章の禁止事項を必ず守る。
...
- Markdown のパースと DOM 処理を分離し、グローバル状態を極力作らない。
- DOM イベントを各所へ散らさず、ナビゲーション責務は 1 か所へまとめる。
- CSS は Custom Properties を使い、テーマ CSS と構造 CSS を分離する。`!important` は原則使わない。クラス名には意味を持たせる。

### パッケージマネージャーと生成物

- パッケージマネージャーは pnpm を使う。`package-lock.json` や `yarn.lock` を作らない。
- `node_modules/`、`dist/`、Playwright のレポートや実行結果は生成物。直接編集せず、入力を直して再生成する。
- 利用できる検証コマンドは `skills/qa/SKILL.md` の表を見る。新しく判明したら同じ表へ追記する。
