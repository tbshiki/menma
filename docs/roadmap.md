...
現時点で動くこと: トップページから手元の Markdown（ファイル / 貼り付け）とサンプルを開ける。原稿と一緒に画像を選べば、相対パスの画像も表示できる。開いた原稿は 16:9 のスライドになり、キーボード・操作ボタン・`#/N` の URL で移動でき、8 レイアウトとテーマ、全画面（`F` キー）が効く。読み込んだ原稿はブラウザに保存され、リロードで同じページから再開できる。余白と進み具合のバーの色は入口画面から設定できる。ページ番号を 2 段階で操作すると入口へ戻れる。エラー画面・印刷・CI は M6。

M0〜M3 が v0.1.0、M4 が v0.2.0、M5 が v0.3.0、表示の調整が v0.4.0。**次は機能追加ではなく、6 章のゲート（実際に 1 回発表して評価する）。**
