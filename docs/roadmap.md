...
| M5 | 画像の取り込み | 原稿と一緒に選んだ画像を表示する | FR-30〜FR-34 | 完了 |
| — | 表示の調整 | 画面いっぱいに広げ、進み具合のバー・色の設定・入口へ戻る操作を足す | FR-09, FR-28, FR-35〜FR-37 | 完了 |
| — | 印刷と PDF | 印刷 CSS を前倒しし、ブラウザから PDF に出せるようにする（[D-23](./decisions.md)） | FR-12 | 完了 |
| M6 | 堅牢性と検証 | 壊れ方を設計し、CI で守る | FR-19〜FR-23, NFR-09 | 未着手 |
| M7 | リリース準備 | 他人が使い始められる状態にする | 受け入れ条件全項目 | 未着手 |

現時点で動くこと: トップページから手元の Markdown（ファイル / 貼り付け）とサンプルを開ける。原稿と一緒に画像を選べば、相対パスの画像も表示できる。開いた原稿は 16:9 のスライドになり、キーボード・操作ボタン・`#/N` の URL で移動でき、8 レイアウトとテーマ、全画面（`F` キー）が効く。読み込んだ原稿はブラウザに保存され、リロードで同じページから再開できる。余白と進み具合のバーの色は入口画面から設定できる。ページ番号を 2 段階で操作すると入口へ戻れる。`Ctrl+P` の印刷から 1 ページ 1 スライドの PDF に出せる。エラー画面・CI は M6。

M0〜M3 が v0.1.0、M4 が v0.2.0、M5 が v0.3.0、表示の調整が v0.4.0、書き方ガイドと印刷（PDF 出力）が v0.5.0。**次は機能追加ではなく、6 章のゲート（実際に 1 回発表して評価する）。**
...
**実際の登壇か社内勉強会で 1 回使う。** 当初は M3 の直後に置いたが、自分の原稿と画像で試せたほうが評価しやすいため、M4（原稿の読み込み）と M5（画像の取り込み）の完了後に実施する（[D-19](./decisions.md)、[D-20](./decisions.md)）。**機能追加はここで止め、評価してから M6 へ進む。**
...
**目的**: 原稿と一緒に選んだ画像を、相対パスの参照へ結び付けて表示する。

M4 と同じく MVP の受け入れ条件には含まれないが、利用者の求めに応じてゲートより先に実施する（[D-20](./decisions.md)）。

タスク

- [x] `deck/source.ts`: `DeckSource` へ画像（`DeckAsset[]`）を持たせる（FR-30）
- [x] `storage/deckStore.ts`: IndexedDB への保存・復元と、`localStorage` からの移行（FR-32）
- [x] `view/assets.ts`: ファイル名での照合、blob URL の生成と解放、`img` と背景画像の差し替え（FR-31）
- [x] `view/home.ts`: 複数ファイルの受け取りと振り分け、上限超過・未解決画像の通知（FR-33、FR-34）
- [x] `app.ts` の起動を非同期化し、後始末で blob URL を解放する
- [x] 単体テスト: ファイル名の照合、上限の判定、保存形式の検証
- [x] E2E: 画像付きで開く、リロードで画像ごと復元、上限超過、未解決画像の通知

完了条件

- `.md` と画像をまとめて選ぶと、相対パスの画像が表示される
- リロードしても画像ごと再開できる
- 10MB を超える画像は取り込まれず、理由が分かる
- 原稿が参照している画像が選ばれていないとき、名前が分かる
...
- [x] 印刷 CSS（FR-12、1 ページ 1 スライド、UI 非表示、transform 無効化）。PDF 出力の手段として前倒しで実施（[D-23](./decisions.md)）
...
| 高 | PDF 出力 | 印刷 CSS で対応済み（[D-23](./decisions.md)）。ワンクリック保存が要るなら生成方式を再検討する |
...
| 中 | フォルダごとの取り込み | いまはファイルを複数選ぶ方式で、ファイル名だけで照合している（[D-20](./decisions.md)）。`webkitdirectory` などでフォルダを扱えば階層まで解決でき、同名の別画像も区別できる |
