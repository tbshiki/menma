...
| — | 表示の調整 | 画面いっぱいに広げ、進み具合のバーを出す | FR-09, FR-35 | 完了 |
| M6 | 堅牢性と検証 | 壊れ方を設計し、CI で守る | FR-19〜FR-23, NFR-09 | 未着手 |
| M7 | リリース準備 | 他人が使い始められる状態にする | 受け入れ条件全項目 | 未着手 |

現時点で動くこと: トップページから手元の Markdown（ファイル / 貼り付け）とサンプルを開ける。原稿と一緒に画像を選べば、相対パスの画像も表示できる。開いた原稿は 16:9 のスライドになり、キーボード・操作ボタン・`#/N` の URL で移動でき、8 レイアウトとテーマ、全画面（`F` キー）が効く。読み込んだ原稿はブラウザに保存され、リロードで同じページから再開できる。エラー画面・印刷・CI は M6。
